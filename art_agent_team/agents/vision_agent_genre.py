...
class VisionAgentGenre(VisionAgentAbstract):
    """Agent for analyzing genre-scene art using Vertex AI Vision APIs."""
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            logging.info("VisionAgentGenre: Vertex AI model initialized successfully")
        except Exception as e:
            logging.error(f"VisionAgentGenre: Failed to initialize Vertex AI model: {e}")
            raise
    ...
    def analyze_image(self, image_path: str, research_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Analyzes genre-scene image using Vertex AI, incorporating research data.
        Focuses on daily life scenes, social interactions, and cultural context.
        """
        try:
            ...
            with Image.open(image_path) as img:
                ...
                # Prepare prompt for genre scene analysis
                prompt = """Analyze this genre scene artwork focusing on:
                1. Daily life activities and social interactions
                2. Setting and environment details
                3. Cultural and historical context
                4. Social relationships and dynamics
                5. Objects of everyday life
                6. Interior/exterior spaces
                
                For each identified element, provide:
                - Description of activity or interaction
                ...
                # Calculate importance scores for genre scene elements
    ...
    def _calculate_object_importance(self, masks: List[SegmentationMask], 
        ...
        """
        Calculate importance scores for objects in genre scene context.
        Prioritizes daily life activities, social interactions, and period elements.
        """
        importance_scores = {}
        ...
        for mask in masks:
            ...
            # Boost score for daily life activities
            if any(keyword in label for keyword in ['activity', 'work', 'leisure', 'domestic']):
                score += 0.4

            # Boost for social interaction elements
            if any(keyword in label for keyword in ['group', 'family', 'social', 'gathering']):
                score += 0.3

            # Boost for period-specific elements
            if any(keyword in label for keyword in ['furniture', 'tool', 'clothing', 'household']):
                score += 0.2

            # Boost for environmental context
            if any(keyword in label for keyword in ['interior', 'exterior', 'room', 'street']):
                score += 0.2

            # Boost for research data matches
            if label in research_data.get('period_elements', []):
                score += 0.2
    ...
    def _threshold_important_objects(self, object_importance: Dict[str, float]) -> List[str]:
        ...
        Uses a dynamic threshold for genre scenes, prioritizing daily life and social elements.
        """
        if not object_importance:
            return []

        # Dynamic threshold for genre scenes
        scores = list(object_importance.values())
        mean_score = np.mean(scores)
        std_score = np.std(scores)
        threshold = mean_score + 0.3 * std_score  # Lower threshold for genre scenes
        
        # Always include key daily life and social elements
        important_objects = [obj for obj, score in object_importance.items()
                           ...
                           any(keyword in obj.lower() for keyword in 
                               ['activity', 'social', 'domestic', 'work'])]
        
        return important_objects

    def _save_labeled_version(self, image_path: str, masks: List[SegmentationMask],
                            analysis: Dict[str, Any]) -> str:
        """Save a version of the image with genre scene elements labeled and annotated."""
        with Image.open(image_path) as img:
            ...
            for idx, mask in enumerate(masks):
                ...
                # Add label with genre-specific details
                details = analysis.get('scene_details', {}).get(label, '')
    ...
    def _save_masked_version(self, image_path: str, masks: List[SegmentationMask]) -> str:
        """Save a version of the image with genre scene elements highlighted using masks."""
        with Image.open(image_path) as img:
